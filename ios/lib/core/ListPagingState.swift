...
class ListPagingState<T: AnyObject>: RandomAccessCollection {
    ...
    private var pagingDataPresenter: SwiftPagingDataPresenter<T>?
    ...
    func update(flow: any SkieSwiftFlowProtocol<Paging_commonPagingData<T>>) {
        stopCollecting()
        let cached: Paging_commonPagingData<T>? =
            flow is SkieSwiftSharedFlow<Paging_commonPagingData<T>>
        ...
        loadState =
            ...
            ?? Paging_commonCombinedLoadStates(
                refresh: initialLoadStates.refresh,
                prepend: initialLoadStates.prepend,
                append: initialLoadStates.append,
                source: initialLoadStates,
    ...
    private func collectPagingData(flow: any SkieSwiftFlowProtocol<Paging_commonPagingData<T>>) async {
        ...
        do {
            for try await pagingData in flow {
                do {
                    try await presenter.collectFrom(
                        pagingData: pagingData as! Paging_commonPagingData<T>)
                } catch {
