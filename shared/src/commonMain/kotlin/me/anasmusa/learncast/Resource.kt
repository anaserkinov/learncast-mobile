...
object Resource {
    ...
    fun setStrings(locale: String, list: List<String>){
        this.strings.clear()
        this.strings.add("")
...
fun Int.string(): String{
    return string(emptyArray<Any?>())
}

fun Int.string(arg: Any?): String{
    return string(args = arrayOf(arg))
}
...
fun Int.quantityString(arg: Any?): String{
    return quantityString(args = arrayOf(arg))
}

//"zero", "one", "two", "few", "many", "other"
fun Int.quantityString(vararg args: Any?): String{
    val quantity = (args[0] as Number).toDouble()
    val offset = if (quantity > 1.0)
        6
    else
        2
...
expect fun readStringFile(locale: String, onLoad: (List<String>) -> Unit)
