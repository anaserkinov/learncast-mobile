...
class LocalDateTimeConverter {
    ...
    @TypeConverter
    fun encode(value: LocalDateTime?): Long? {
        return value?.toInstant(TimeZone.currentSystemDefault())?.toEpochMilliseconds()
    }

    @OptIn(ExperimentalTime::class)
    @TypeConverter
    fun decode(value: Long?): LocalDateTime?{
        return value?.let { Instant.fromEpochMilliseconds(it).toDateTime() }
    }
}

class DurationConverter {
    @TypeConverter
    fun encode(value: Duration?): Long? {
        return value?.inWholeMilliseconds
    }

    @TypeConverter
    fun decode(value: Long?): Duration?{
