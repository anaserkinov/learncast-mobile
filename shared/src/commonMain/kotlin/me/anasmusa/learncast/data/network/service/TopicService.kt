...
internal class TopicService(private val client: HttpClient) {

    suspend fun page(requestQuery: PageRequestQuery) =  client.get("v1/user/topic"){
        headers.append(HttpHeaders.CacheControl, "no-cache")
        requestQuery.load(url)
    }.body<BaseResponse<PageResponse<TopicResponse>>>()

    suspend fun deleted(requestQuery: DeletedRequestQuery) =  client.get("v1/user/topic/deleted"){
        requestQuery.load(url)
