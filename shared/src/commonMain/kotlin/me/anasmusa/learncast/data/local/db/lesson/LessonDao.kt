...
internal interface LessonDao {
    ...
    suspend fun updateUserSnipCount(lessonId: Long, count: Long)

    @Query("UPDATE ${TableNames.LESSON_STATE} SET listenCount = :count WHERE lessonId = :lessonId")
    suspend fun updateListenCount(lessonId: Long, count: Long)
    ...
    fun getLessons(
        ...
        query.append(", s.listenCount AS state_listenCount")
        ...
        if (sort != null){
            val order = if (order == QueryOrder.ASC) "ASC"
            else "DESC"
            when(sort){
    ...
    fun getDownloadedLessons(
        ...
        query.append(" FROM ${TableNames.DOWNLOAD_STATE} AS d")
