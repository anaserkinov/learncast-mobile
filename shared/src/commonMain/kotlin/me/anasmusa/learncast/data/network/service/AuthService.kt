...
internal class AuthService(private val client: HttpClient) {
    ...
    suspend fun login(request: LoginRequest) =  client.post("v1/user/auth/signin"){
        setBody(request)
    }.body<BaseResponse<LoginResponse>>()

    suspend fun refreshToken(request: RefreshTokenRequest) = client.post("v1/user/auth/refresh-token"){
        attributes.put(AuthCircuitBreaker, Unit)
        setBody(request)
    }.body<BaseResponse<Credentials>>()

    suspend fun logout(){
        client.authProvider<BearerAuthProvider>()
            ?.clearToken()
